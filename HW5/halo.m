...
for idx = 1:length(M_lst)
    ...
    derivative
    if idx == 1
        n_ps_1(idx) = -(2/pi)^(1/2)*deltac/sigmavalue^2*rhom/M*exp(-deltac^2/2/sigmavalue^2)*derivative*10^9*(M_lst(idx+1)-M_lst(idx));
    elseif idx == length(M_lst)
        n_ps_1(idx) = -(2/pi)^(1/2)*deltac/sigmavalue^2*rhom/M*exp(-deltac^2/2/sigmavalue^2)*derivative*10^9*(M_lst(idx)-M_lst(idx-1));
    else
        n_ps_1(idx) = -(2/pi)^(1/2)*deltac/sigmavalue^2*rhom/M*exp(-deltac^2/2/sigmavalue^2)*derivative*10^9*((M_lst(idx+1)-M_lst(idx-1))/2);
    end
end
