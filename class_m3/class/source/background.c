...
int background_functions(
  ...
  /* scalar field dark matter 3 quantities */
  /*Polar variables arXiv: 1511.08195*/
  double theta_sfdm_3, y1_sfdm_3, alpha_sfdm_3;
  ...
  /*3 Scalar Field Dark Matter*/
  if((pba->has_sfdm_1 == _TRUE_) && (pba->has_sfdm_2 == _TRUE_) && (pba->has_sfdm_3 == _TRUE_)){
    /*SFDM1 auxiliaries*/
    ...
    /*SFDM3 auxiliaries*/
    theta_sfdm_3 = pvecback_B[pba->index_bi_theta_sfdm_3];
    y1_sfdm_3 = pvecback_B[pba->index_bi_y1_sfdm_3];
    alpha_sfdm_3 = pvecback_B[pba->index_bi_alpha_sfdm_3];
    /*SFDM1 quantities*/
    ...
    pvecback[pba->index_bg_rho_sfdm_1] = exp(alpha_sfdm_1)*rho_tot/(1.-exp(alpha_sfdm_1)-exp(alpha_sfdm_2)-exp(alpha_sfdm_3)); // energy of the sfdm.
    ...
    pvecback[pba->index_bg_rho_sfdm_2] = exp(alpha_sfdm_2)*rho_tot/(1.-exp(alpha_sfdm_1)-exp(alpha_sfdm_2)-exp(alpha_sfdm_3)); // energy of the sfdm.
    ...
    /*SFDM3 quantities*/
    pvecback[pba->index_bg_theta_sfdm_3] = theta_sfdm_3; // value of the angular variable
    pvecback[pba->index_bg_y1_sfdm_3] = y1_sfdm_3; // value of the potential variable
    pvecback[pba->index_bg_alpha_sfdm_3] = alpha_sfdm_3; // value of 0.5*log(Omega_sfdm)
    pvecback[pba->index_bg_rho_sfdm_3] = exp(alpha_sfdm_3)*rho_tot/(1.-exp(alpha_sfdm_1)-exp(alpha_sfdm_2)-exp(alpha_sfdm_3)); // energy of the sfdm.
    pvecback[pba->index_bg_p_sfdm_3] = -cos_sfdm(pba,theta_sfdm_3)*pvecback[pba->index_bg_rho_sfdm_3]; // pressure of the sfdm
    rho_m += pvecback[pba->index_bg_rho_sfdm_3] - 3.*pvecback[pba->index_bg_p_sfdm_3];
    rho_r += 3.*pvecback[pba->index_bg_p_sfdm_3];
    /*Total quantities*/
    rho_tot += pvecback[pba->index_bg_rho_sfdm_1]+pvecback[pba->index_bg_rho_sfdm_2]+pvecback[pba->index_bg_rho_sfdm_3];
    p_tot += pvecback[pba->index_bg_p_sfdm_1]+pvecback[pba->index_bg_p_sfdm_2]+pvecback[pba->index_bg_p_sfdm_3];
  }
  /*Individual SFDM*/
  else{
  /* Scalar field dark matter 1/2.*/
  if ((pba->has_sfdm_1 == _TRUE_) && (pba->has_sfdm_2 == _TRUE_) && (pba->has_sfdm_3 == _FALSE_)){
    theta_sfdm_1 = pvecback_B[pba->index_bi_theta_sfdm_1];
    y1_sfdm_1 = pvecback_B[pba->index_bi_y1_sfdm_1];
    alpha_sfdm_1 = pvecback_B[pba->index_bi_alpha_sfdm_1];
    pvecback[pba->index_bg_theta_sfdm_1] = theta_sfdm_1; // value of the angular variable
  ...
  /* Scalar field dark matter 2/3.*/
  if ((pba->has_sfdm_2 == _TRUE_) && (pba->has_sfdm_3 == _TRUE_) && (pba->has_sfdm_2 == _FALSE_)){
    theta_sfdm_2 = pvecback_B[pba->index_bi_theta_sfdm_2];
    y1_sfdm_2 = pvecback_B[pba->index_bi_y1_sfdm_2];
    alpha_sfdm_2 = pvecback_B[pba->index_bi_alpha_sfdm_2];
    pvecback[pba->index_bg_theta_sfdm_2] = theta_sfdm_2; // value of the angular variable
    pvecback[pba->index_bg_y1_sfdm_2] = y1_sfdm_2; // value of the potential variable
    pvecback[pba->index_bg_alpha_sfdm_2] = alpha_sfdm_2; // value of 0.5*log(Omega_sfdm)
    pvecback[pba->index_bg_rho_sfdm_2] = exp(alpha_sfdm_2)*rho_tot/(1.-exp(alpha_sfdm_2)-exp(alpha_sfdm_3)); // energy of the sfdm.
    pvecback[pba->index_bg_p_sfdm_2] = -cos_sfdm(pba,theta_sfdm_2)*pvecback[pba->index_bg_rho_sfdm_2]; // pressure of the sfdm
    rho_m += pvecback[pba->index_bg_rho_sfdm_2] - 3.*pvecback[pba->index_bg_p_sfdm_2];
    rho_r += 3.*pvecback[pba->index_bg_p_sfdm_2];
  ...
  /* Scalar field dark matter 3/1.*/
  if ((pba->has_sfdm_3 == _TRUE_) && (pba->has_sfdm_1 == _TRUE_) && (pba->has_sfdm_2 == _FALSE_)){
    theta_sfdm_3 = pvecback_B[pba->index_bi_theta_sfdm_3];
    y1_sfdm_3 = pvecback_B[pba->index_bi_y1_sfdm_3];
    alpha_sfdm_3 = pvecback_B[pba->index_bi_alpha_sfdm_3];
    pvecback[pba->index_bg_theta_sfdm_3] = theta_sfdm_3; // value of the angular variable
    pvecback[pba->index_bg_y1_sfdm_3] = y1_sfdm_3; // value of the potential variable
    pvecback[pba->index_bg_alpha_sfdm_3] = alpha_sfdm_3; // value of 0.5*log(Omega_sfdm)
    pvecback[pba->index_bg_rho_sfdm_3] = exp(alpha_sfdm_3)*rho_tot/(1.-exp(alpha_sfdm_3)-exp(alpha_sfdm_1)); // energy of the sfdm.
    pvecback[pba->index_bg_p_sfdm_3] = -cos_sfdm(pba,theta_sfdm_3)*pvecback[pba->index_bg_rho_sfdm_3]; // pressure of the sfdm
    rho_m += pvecback[pba->index_bg_rho_sfdm_3] - 3.*pvecback[pba->index_bg_p_sfdm_3];
    rho_r += 3.*pvecback[pba->index_bg_p_sfdm_3];
    rho_tot += pvecback[pba->index_bg_rho_sfdm_3];
    p_tot += pvecback[pba->index_bg_p_sfdm_3];
  }
  /* Scalar field dark matter 1.*/
  if ((pba->has_sfdm_1 == _TRUE_) && (pba->has_sfdm_2 == _FALSE_) && (pba->has_sfdm_3 == _FALSE_)){
    theta_sfdm_3 = pvecback_B[pba->index_bi_theta_sfdm_3];
    y1_sfdm_3 = pvecback_B[pba->index_bi_y1_sfdm_3];
    alpha_sfdm_3 = pvecback_B[pba->index_bi_alpha_sfdm_3];
    pvecback[pba->index_bg_theta_sfdm_3] = theta_sfdm_3; // value of the angular variable
    pvecback[pba->index_bg_y1_sfdm_3] = y1_sfdm_3; // value of the potential variable
    pvecback[pba->index_bg_alpha_sfdm_3] = alpha_sfdm_3; // value of 0.5*log(Omega_sfdm)
    pvecback[pba->index_bg_rho_sfdm_3] = exp(alpha_sfdm_3)*rho_tot/(1.-exp(alpha_sfdm_1)); // energy of the sfdm.
    pvecback[pba->index_bg_p_sfdm_3] = -cos_sfdm(pba,theta_sfdm_3)*pvecback[pba->index_bg_rho_sfdm_3]; // pressure of the sfdm
    rho_m += pvecback[pba->index_bg_rho_sfdm_3] - 3.*pvecback[pba->index_bg_p_sfdm_3];
    rho_r += 3.*pvecback[pba->index_bg_p_sfdm_3];
    rho_tot += pvecback[pba->index_bg_rho_sfdm_3];
  ...
  if ((pba->has_sfdm_2 == _TRUE_) && (pba->has_sfdm_1 == _FALSE_) && (pba->has_sfdm_3 == _FALSE_)){
    theta_sfdm_3 = pvecback_B[pba->index_bi_theta_sfdm_3];
    y1_sfdm_3 = pvecback_B[pba->index_bi_y1_sfdm_3];
    alpha_sfdm_3 = pvecback_B[pba->index_bi_alpha_sfdm_3];
    pvecback[pba->index_bg_theta_sfdm_3] = theta_sfdm_3; // value of the angular variable
    pvecback[pba->index_bg_y1_sfdm_3] = y1_sfdm_3; // value of the potential variable
    pvecback[pba->index_bg_alpha_sfdm_3] = alpha_sfdm_3; // value of 0.5*log(Omega_sfdm)
    pvecback[pba->index_bg_rho_sfdm_3] = exp(alpha_sfdm_3)*rho_tot/(1.-exp(alpha_sfdm_2)); // energy of the sfdm.
    pvecback[pba->index_bg_p_sfdm_3] = -cos_sfdm(pba,theta_sfdm_3)*pvecback[pba->index_bg_rho_sfdm_3]; // pressure of the sfdm
    rho_m += pvecback[pba->index_bg_rho_sfdm_3] - 3.*pvecback[pba->index_bg_p_sfdm_3];
    rho_r += 3.*pvecback[pba->index_bg_p_sfdm_3];
    rho_tot += pvecback[pba->index_bg_rho_sfdm_3];
  ...
  if ((pba->has_sfdm_3 == _TRUE_) && (pba->has_sfdm_1 == _FALSE_)&& (pba->has_sfdm_2 == _FALSE_)){
    theta_sfdm_3 = pvecback_B[pba->index_bi_theta_sfdm_3];
    y1_sfdm_3 = pvecback_B[pba->index_bi_y1_sfdm_3];
    alpha_sfdm_3 = pvecback_B[pba->index_bi_alpha_sfdm_3];
    pvecback[pba->index_bg_theta_sfdm_3] = theta_sfdm_3; // value of the angular variable
    pvecback[pba->index_bg_y1_sfdm_3] = y1_sfdm_3; // value of the potential variable
    pvecback[pba->index_bg_alpha_sfdm_3] = alpha_sfdm_3; // value of 0.5*log(Omega_sfdm)
    pvecback[pba->index_bg_rho_sfdm_3] = exp(alpha_sfdm_3)*rho_tot/(1.-exp(alpha_sfdm_3)); // energy of the sfdm.
    pvecback[pba->index_bg_p_sfdm_3] = -cos_sfdm(pba,theta_sfdm_3)*pvecback[pba->index_bg_rho_sfdm_3]; // pressure of the sfdm
    rho_m += pvecback[pba->index_bg_rho_sfdm_3] - 3.*pvecback[pba->index_bg_p_sfdm_3];
    rho_r += 3.*pvecback[pba->index_bg_p_sfdm_3];
    rho_tot += pvecback[pba->index_bg_rho_sfdm_3];
  ...
  pvecback[pba->index_bg_w_tot_3] = p_tot/rho_tot;
...
int background_free_input(
  ...
  if (pba->Omega0_ncdm_tot != 0.){
    ...
    /*Scalar field dark matter 3*/
    if (pba->Omega0_sfdm_3 != 0.){
        if (pba->sfdm_parameters_3 != NULL)
        free(pba->sfdm_parameters_3);
    }
  if (pba->Omega0_scf != 0.){
...
int background_indices(
  ...
  class_define_index(pba->index_bg_w_tot_3,_TRUE_,index_bg,1);
  ...
  class_define_index(pba->index_bg_rho_dr,pba->has_dr,index_bg,1);
    ...
    /* - indices for scalar field dark matter 3*/
    class_define_index(pba->index_bg_theta_sfdm_3,pba->has_sfdm_3,index_bg,1);
    class_define_index(pba->index_bg_y1_sfdm_3,pba->has_sfdm_3,index_bg,1);
    class_define_index(pba->index_bg_alpha_sfdm_3,pba->has_sfdm_3,index_bg,1);
    class_define_index(pba->index_bg_rho_sfdm_3,pba->has_sfdm_3,index_bg,1);
    class_define_index(pba->index_bg_p_sfdm_3,pba->has_sfdm_3,index_bg,1);
  ...
  class_define_index(pba->index_bi_rho_fld,pba->has_fld,index_bi,1);
    ...
    /* -> scalar field dark matter 3 quantities */
    class_define_index(pba->index_bi_theta_sfdm_3,pba->has_sfdm_3,index_bi,1);
    class_define_index(pba->index_bi_y1_sfdm_3,pba->has_sfdm_3,index_bi,1);
    class_define_index(pba->index_bi_alpha_sfdm_3,pba->has_sfdm_3,index_bi,1);
  ...
  //if (pba->background_verbose > 0) {
    ...
    if (pba->has_sfdm_1 == _TRUE_){
      ...
      if (pba->has_sfdm_3 == _TRUE_){
        printf("Scalar Field Dark Matter 3 details:\n");
        printf(" -> Omega_sfdm_3 = %g, wished %g\n",
                 exp(pvecback[pba->index_bg_alpha_sfdm_3]), pba->Omega0_sfdm_3);
        printf(" -> lambda_sfdm_3 = %1.2e\n", pba->sfdm_parameters_3[1]);
        /*Print quadratic and cos mass information*/
        if (pba->sfdm_parameters_3[1] >= 0.){
        printf(" -> Mass_sfdm_3 = %1.2e [eV], %1.2e [1/Mpc], %1.2e [H_0]\n",
                    3.19696e-30*pvecback[pba->index_bg_y1_sfdm_3]*pvecback[pba->index_bg_H], 0.5*pvecback[pba->index_bg_y1_sfdm_3]*pvecback[pba->index_bg_H], 0.5*pvecback[pba->index_bg_y1_sfdm_3]);
        }
        /*Print cosh mass information*/
        else{
          printf(" -> Mass_sfdm_3 = %1.2e [eV], %1.2e [1/Mpc], %1.2e [H_0]\n",
                      3.19696e-30*exp(pvecback[pba->index_bg_y1_sfdm_3])*pvecback[pba->index_bg_H], 0.5*exp(pvecback[pba->index_bg_y1_sfdm_3])*pvecback[pba->index_bg_H], 0.5*exp(pvecback[pba->index_bg_y1_sfdm_3]));
        }
        printf("    -> wished = %1.2e [eV]\n", pow(10.,pba->sfdm_parameters_3[0]));
        }
////////////////////////////////////////////////////////////////
    if(pba->has_lambda == _TRUE_)
...
int background_initial_conditions(
  ...
  */
  ...
  if(pba->has_sfdm_2 == _TRUE_){
    ...
    /** - Fix initial values of variables of sfdm 3*/
  if(pba->has_sfdm_3 == _TRUE_){
    //sfdm_lambda = pba->sfdm_parameters[0];
    pvecback_integration[pba->index_bi_theta_sfdm_3] = pba->theta_ini_sfdm_3;
    pvecback_integration[pba->index_bi_y1_sfdm_3] = pba->y1_ini_sfdm_3;
    pvecback_integration[pba->index_bi_alpha_sfdm_3] = pba->alpha_ini_sfdm_3;

    class_test(!isfinite(pvecback_integration[pba->index_bi_theta_sfdm_3]) ||
    !isfinite(pvecback_integration[pba->index_bi_theta_sfdm_3]),
    pba->error_message,
    "initial theta_sfdm_3 = %e alpha_sfdm_3 = %e -> check initial conditions",
    pvecback_integration[pba->index_bi_theta_sfdm_3],
    pvecback_integration[pba->index_bi_alpha_sfdm_3]);
  }
...
int background_output_titles(struct background * pba,
  ...
  /*Scalar Field Dark Matter 3*/
  class_store_columntitle(titles,"(.)rho_sfdm_3",pba->has_sfdm_3);
  class_store_columntitle(titles,"(.)p_sfdm_3",pba->has_sfdm_3);
  class_store_columntitle(titles,"theta_sfdm_3",pba->has_sfdm_3);
  class_store_columntitle(titles,"y1_sfdm_3",pba->has_sfdm_3);
  class_store_columntitle(titles,"alpha_sfdm_3",pba->has_sfdm_3);
...
int background_output_data(
  ...
  for (index_tau=0; index_tau<pba->bt_size; index_tau++){
    ...
    /*Scalar Field Dark Matter 3*/
    class_store_double(dataptr,pvecback[pba->index_bg_rho_sfdm_3],pba->has_sfdm_3,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_p_sfdm_3],pba->has_sfdm_3,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_theta_sfdm_3],pba->has_sfdm_3,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_y1_sfdm_3],pba->has_sfdm_3,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_alpha_sfdm_3],pba->has_sfdm_3,storeidx);
...
int background_derivs(
  ...
  /*Differential equations for Scalar Fiel Dark Matter 3*/
  if (pba->has_sfdm_3 == _TRUE_){
    /** - Scalar field dark matter EoM */
    /*Quadratic and cos*/
    if (pba->sfdm_parameters_3[1] >= 0.){
    dy[pba->index_bi_alpha_sfdm_3] = 3.*y[pba->index_bi_a]*pvecback[pba->index_bg_H]*
      (pvecback[pba->index_bg_w_tot_3]+cos_sfdm(pba,y[pba->index_bi_theta_sfdm_3]));
    dy[pba->index_bi_theta_sfdm_3] = y[pba->index_bi_a]*pvecback[pba->index_bg_H]*
      (-3.*sin_sfdm(pba,y[pba->index_bi_theta_sfdm_3])+y[pba->index_bi_y1_sfdm_3]);
    dy[pba->index_bi_y1_sfdm_3] = y[pba->index_bi_a]*pvecback[pba->index_bg_H]*
      (1.5*(1.+pvecback[pba->index_bg_w_tot_3])*y[pba->index_bi_y1_sfdm_3]
      + y2_sfdm(pba, y[pba->index_bi_alpha_sfdm_3], y[pba->index_bi_theta_sfdm_3], y[pba->index_bi_y1_sfdm_3], pba->sfdm_parameters_3[1])*
      exp(0.5*y[pba->index_bi_alpha_sfdm_3])*sin_sfdm(pba,0.5*y[pba->index_bi_theta_sfdm_3]));
    }
    /*Cosh*/
    else{
      dy[pba->index_bi_alpha_sfdm_3] = 3.*y[pba->index_bi_a]*pvecback[pba->index_bg_H]*
      ...
      dy[pba->index_bi_theta_sfdm_3] = y[pba->index_bi_a]*pvecback[pba->index_bg_H]*
        (-3.*sin_sfdm(pba,y[pba->index_bi_theta_sfdm_3])+
        exp(y[pba->index_bi_y1_sfdm_3])*
        pow(1.-pba->sfdm_parameters_3[1]*exp(y[pba->index_bi_alpha_sfdm_3]-2.*y[pba->index_bi_y1_sfdm_3])
           *(1.+cos_sfdm(pba,y[pba->index_bi_theta_sfdm_3])),0.5));
              //pow(pow(y[pba->index_bi_y_phi_scf],2.)-pba->scf_parameters[0]*exp(y[pba->index_bi_Omega_phi_scf])*(1.+cos_scf(pba,y[pba->index_bi_theta_phi_scf])),0.5));
      //General expression for: axion (lambda >0), quadratic (lambda =0), cosh (lambda < 0)
       dy[pba->index_bi_y1_sfdm_3] = 1.5*y[pba->index_bi_a]*pvecback[pba->index_bg_H]*
       (1.+pvecback[pba->index_bg_w_tot_3]);
     }
