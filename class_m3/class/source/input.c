...
int input_init(
  ...
  char * const target_namestrings[] = {"100*theta_s","Omega_dcdmdr","omega_dcdmdr","Omega_sfdm_1", "Omega_sfdm_2", "Omega_sfdm_3",
                                       "Omega_scf","Omega_ini_dcdm","omega_ini_dcdm","sigma8"};
  char * const unknown_namestrings[] = {"h","Omega_ini_dcdm","Omega_ini_dcdm","sfdm_shooting_parameter_1", "sfdm_shooting_parameter_2", "sfdm_shooting_parameter_3",
...
int input_read_parameters(
  ...
  double theta_ini_3, alpha_ini_3; //Auxiliar variables for cos potential
  double theta_sfdm_3, alpha_sfdm_3, y1_sfdm_3;
  double aosc_3, aosc3_3, b3_3; /*aguess1_2, aguess2_2; //Auxiliar variables for cos potential*/
  double theta0_3, theta1_3, theta2_3, theta3_3, alpha1_3, alpha2_3, alpha3_3; //Auxiliar variables for cos potential
  double masstohubble_ini_3, y1_aux_sfdm_3; /*Auxiliar variables for cosh*/
  /**/
  ...
  /*--------------------------------------------------------*/
  /*Scalar Field Dark Matter 3 Read the current value of Omega_sfdm_3 from the .ini archive*/
  /*--------------------------------------------------------*/
  
  /** - Omega_0_sfdm_3 (SFCDM) */
  class_call(parser_read_double(pfc,"Omega_sfdm_3",&param1,&flag1,errmsg),
             errmsg,
             errmsg);
  class_call(parser_read_double(pfc,"omega_sfdm_3",&param2,&flag2,errmsg),
             errmsg,
             errmsg);
  class_test(((flag1 == _TRUE_) && (flag2 == _TRUE_)),
             errmsg,
             "In input file, you can only enter one of Omega_sfdm_3 or omega_sfdm_3, choose one");
  if (flag1 == _TRUE_)
    pba->Omega0_sfdm_3 = param1;
  if (flag2 == _TRUE_)
    pba->Omega0_sfdm_3 = param2/pba->h/pba->h;
  
  Omega_tot += pba->Omega0_sfdm_3;
  /* Additional SFDM 3 parameters: */
  if (pba->Omega0_sfdm_3 != 0.){
    /** - Read parameters describing scalar field potential */
    class_call(parser_read_list_of_doubles(pfc,
                                           "sfdm_parameters_3",
                                           &(pba->sfdm_parameters_size_3),
                                           &(pba->sfdm_parameters_3),
                                           &flag1,
                                           errmsg),
               errmsg,errmsg);
    class_read_int("sfdm_tuning_index_3",pba->sfdm_tuning_index_3);
    class_test(pba->sfdm_tuning_index_3 >= pba->sfdm_parameters_size_3,
               errmsg,
               "Tuning index sfdm_tuning_index_3 = %d is larger than the number of entries %d in sfdm_parameters_3. Check your .ini file.",pba->sfdm_tuning_index_3,pba->sfdm_parameters_size_3);
    /** - Assign shooting parameter */
    class_read_double("sfdm_shooting_parameter_3",pba->sfdm_parameters_3[pba->sfdm_tuning_index_3]);
    /*Initial conditions for sfdm_3*/
    if (pba->sfdm_parameters_3[1] >= 0.){
      theta_ini_3 = 0.4*15.64*pow(10.,pba->sfdm_parameters_3[0])/(pow(pba->Omega0_g+pba->Omega0_ur,0.5)*pba->H0);
      /*Newton-Raphson to solve the cubic correction for aosc when lambda > 0.*/
      aosc_3 = pow((0.5*_PI_/theta_ini_3)/pow(1.+pow(_PI_,2)/36.,0.5),0.5);
      b3_3 = 1.e-14*pba->sfdm_parameters_3[1]*pba->Omega0_sfdm_3/(72.*(pba->Omega0_g+pba->Omega0_ur));
      aosc3_3 = pow(aosc_cubic(aosc_3, b3_3),3.);
      /*Calcule pivot value of Omega_ini to calcule appropiate initial conditions.*/
      alpha_ini_3 = pba->sfdm_parameters_3[pba->sfdm_tuning_index_3] + log(pba->Omega0_sfdm_3*1.e-14/(pow(aosc_3,3.)*(pba->Omega0_g+pba->Omega0_ur)));
      /*Secant method to find the third correction for theta_sfdm*/
      if(pba->sfdm_parameters_3[1] > 0.){
        theta1_3 = theta_ini_3;
        alpha1_3 = alpha_ini_3;
        theta2_3 = (36.*_PI_/pow(pba->sfdm_parameters_3[1],2.))*1.e28*pow(2.5*theta1_3*(pba->Omega0_g+pba->Omega0_ur)/pba->Omega0_sfdm_3,2.);
        aosc_3 = pow((0.5*_PI_/theta2_3)/pow(1. + pow(_PI_,2)/36.,0.5),0.5);
        aosc3_3 = pow(aosc_cubic(aosc_3,b3_3),3.);
        alpha2_3 = log(pba->Omega0_sfdm_3*1e-14/(aosc3_3*(pba->Omega0_g+pba->Omega0_ur)));
        for(i=0; i<30; i++){
          theta3_3 = theta1_3 - (theta1_3 - theta2_3)*fmass(theta1_3, alpha1_3, theta_ini_3, pba->sfdm_parameters_3[1])/
          (fmass(theta1_3, alpha1_3, theta_ini_3, pba->sfdm_parameters_3[1])- fmass(theta2_3, alpha2_3, theta_ini_3, pba->sfdm_parameters_3[1]));
          aosc_3 = pow((0.5*_PI_/theta3_3)/pow(1.+pow(_PI_,2)/36.,0.5),0.5);
          aosc3_3 = pow(aosc_cubic(aosc_3, b3_3),3.);
          alpha3_3 = pba->sfdm_parameters_3[pba->sfdm_tuning_index_3] + log(pba->Omega0_sfdm_3*1.e-14/(aosc3_3*(pba->Omega0_g+pba->Omega0_ur)));
          if(pow(pow(verify(theta3_3, alpha3_3, theta_ini_3, pba->sfdm_parameters_3[1]),2.),0.5)<1.e-6) break;
          theta1_3 = theta2_3;
          alpha1_3 = alpha2_3;
          theta2_3 = theta3_3;
          alpha2_3 = alpha3_3;
        }
        theta_ini_3 = theta3_3;
        alpha_ini_3 = alpha3_3;
      }
      pba->alpha_ini_sfdm_3 = alpha_ini_3;
      pba->theta_ini_sfdm_3 = theta_ini_3;
    }
    //Cosh
    else{
      theta_ini_3 = acos(-1./3.);
      alpha_ini_3 = log(-12./pba->sfdm_parameters_3[1]);
      masstohubble_ini_3 = 1.e-28*pow(pba->sfdm_parameters_3[1]/3.-4.,2.)*pow(pba->Omega0_sfdm_3/(pba->Omega0_g+pba->Omega0_ur),2.);
      //Now y1, instead of alpha will be adjusted using shooting parameter.
      y1_aux_sfdm_3 = log(2.*masstohubble_ini_3) + pba->sfdm_parameters_3[pba->sfdm_tuning_index_3];
      
      pba->alpha_ini_sfdm_3 = alpha_ini_3;
      pba->theta_ini_sfdm_3 = theta_ini_3;
      pba->y1_ini_sfdm_3 = y1_aux_sfdm_3;
    }
    class_call(parser_read_string(pfc,
                                  "attractor_ic_sfdm_3",
                                  &string1,
    ...
    if(flag1 == _TRUE_){
      if((strstr(string1,"y") != NULL) || (strstr(string1,"Y") != NULL)){
        pba->attractor_ic_sfdm_3 = _TRUE_;
        if(pba->sfdm_parameters_3[1] >= 0.)
          pba->y1_ini_sfdm_3 = 5.*pba->theta_ini_sfdm_3;
        else //cosh
          pba->y1_ini_sfdm_3 = y1_aux_sfdm_3;
        /*else
         pba->y1_ini_sfdm_2 = pow(8.,0.5);*/
        ...
        pba->attractor_ic_sfdm_3 = _FALSE_;
        class_test(pba->sfdm_parameters_size_3<2,
                   errmsg,
                   "Since you are not using the attractor initial conditions,you must specify theta_3 and Omega_sfdm_3 as the last two entries in sfdm_parameters_3. See explanatory.ini for more details.");
        pba->y1_ini_sfdm_3 = pba->sfdm_parameters_3[3];
      }
    }
  }
...
int input_default_params(
  ...
  /*Scalar Field Dark Matter 3 defaults*/
  pba->Omega0_sfdm_3 = 0.; /* Scalar field dark matter defaults */
  pba->attractor_ic_sfdm_3 = _TRUE_;
  pba->sfdm_parameters_3 = NULL;
  pba->sfdm_parameters_size_3 = 0;
  pba->sfdm_tuning_index_3 = 2;
  pba->theta_ini_sfdm_3 = 0.;
  pba->y1_ini_sfdm_3 = 0.;
  pba->alpha_ini_sfdm_3 = 0.;
  //pba->expo_sfdm_3 = 2;
...
int input_try_unknown_parameters(double * unknown_parameter,
  ...
  for (i=0; i < pfzw->target_size; i++) {
    ...
    /*Scalar Field Dark Matter 3*/
    case Omega_sfdm_3:
      output[i] = ba.background_table[(ba.bt_size-1)*ba.bg_size+ba.index_bg_rho_sfdm_3]/(ba.H0*ba.H0)
        -ba.Omega0_sfdm_3;
      break;
    /**   case omega_sfdm:
...
int input_get_guess(double *xguess,
  ...
  for (index_guess=0; index_guess < pfzw->target_size; index_guess++) {
    ...
    case Omega_sfdm_3:
      /* Default: take the passed value as xguess and set dxdy to 1. */
      xguess[index_guess] = ba.sfdm_parameters_3[ba.sfdm_tuning_index_3];
      dxdy[index_guess] = 1.;
      break;
    /*    case omega_sfdm:
